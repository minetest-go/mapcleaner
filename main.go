...
func main() {
	...
	mode := flag.String("mode", "prune_unprotected", "set the working mode [prune_unprotected|prune_unprotected_batched (experimental)|export_protected]")
	...
	// start main process
	switch *mode {
	case "prune_unprotected":
		err = ProcessRemoveUnprotected()
	case "prune_unprotected_batched":
		err = ProcessRemoveUnprotectedBatched()
